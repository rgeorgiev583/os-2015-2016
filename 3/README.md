Basic bash exercises
====================

Using the commands learned so far, write a bash command to:
...
3. Output all groups in which the `daemon` user participates (group info is
   stored in `/etc/group`).
4. Output the pseudo-terminal names for each `bash` instance (use the output of
   the `ps ax` command line).

[]()

Vi Cheat Sheet {#vi-cheat-sheet align="CENTER"}
==============
